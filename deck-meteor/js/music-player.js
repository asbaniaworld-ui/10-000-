/** 点击即播 — Deezer / iTunes 30s 预览 · Spotify 兜底 */
window.DeckMusic = (() => {
  ...
  function enrich(track) {
    ...
    const base =
      catalog.by_genre?.[track.genreKey || track.genre] ||
  ...
  async function play(track) {
    ...
    const preview = await resolvePreviewUrl(t);
    if (preview && (await playAudio(t, preview))) return;
  ...
  function preview(track) {
    const t = enrich(track);
    if (!t?.preview_url && !t?.spotify_url) return;
    clearTimeout(previewTimer);
    clearTimeout(hideTimer);
    previewTimer = setTimeout(() => play(t), 380);
  }
  ...
  function stopPreview() {
    ...
    hideTimer = setTimeout(() => {
      if (document.querySelector("#music-bar:hover")) return;
      stop();
    }, 600);
  }
  ...
  async function load() {
    try {
      ...
      const res = await fetch(`${base}featured_tracks.json?v=111`);
    ...
    bar()?.addEventListener("mouseleave", () => stopPreview());
  }
